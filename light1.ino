...
void HandleLightOff(const char *event, const char *data)
{
    ...
    Particle.publish("yaara_light1_catched_off_2");
    if (light_state == HIGH) {
        set_color(0);
    }
...
void HandleLightOn(const char *event, const char *data)
{
    ...
    strcpy(rxData, data);
    //light2_state = digitalRead(led1);
    Particle.publish("yaara_light1_catched_on_2");
...
void setup()
{
    ...
    Particle.subscribe("yaara_light_light2_on", HandleLightOn);
    Particle.subscribe("yaara_light_light2_off", HandleLightOff);
    //Particle.publish("light1_state", myvar);
    Particle.publish("yaara_light_power_on_1");
...
void loop() {
    if (digitalRead(button1) == LOW) {
        ...
        if (digitalRead(button1) == LOW) {
            set_color(0);
            Particle.publish("yaara_light_light1_off",color_num_str,PRIVATE);
        } else {
            //light2_state = digitalRead(led1);
            set_color(color_num);
            ...
            Particle.publish("yaara_light_light1_on",color_num_str,PRIVATE);
        }
        delay(1000);
    }
    delay(500);
